...
<style scoped>
.play-bar-wrapper{position:fixed;bottom:-70px;left:50%;transform:translateX(-50%);width:90%;max-width:800px;z-index:999;transition:bottom .4s cubic-bezier(.25,.8,.25,1)}
.play-bar-wrapper:hover,.play-bar-wrapper.show{bottom:20px}
.play-bar{width:100%;min-height:80px;display:flex;align-items:center;padding:15px 20px;border:1px solid var(--color-border);background:var(--color-card);box-shadow:0 5px 15px rgba(0,0,0,.2);border-radius:15px 15px 0 0;position:relative}
.play-bar::before{content:'';position:absolute;top:-10px;left:50%;transform:translateX(-50%);width:60px;height:10px;background:var(--color-card);border:1px solid var(--color-border);border-bottom:none;border-radius:10px 10px 0 0}
.play-bar-wrapper:hover .play-bar::before{opacity:0}
.song-info-container{display:flex;align-items:center;flex:1;margin-right:20px;min-width:0;cursor:pointer;position:relative}
...
.song-info{flex:1;min-width:0;position:relative}
...
.current-song-progress-wrapper{position:absolute;bottom:0;left:0;width:100%;height:4px;pointer-events:none}
.current-song-progress-bg{width:100%;height:100%;background:rgba(0,0,0,.15);border-radius:2px;overflow:hidden}
.current-song-progress-fill{height:100%;background:var(--color-btn);border-radius:2px;transition:width .2s linear}
.progress-section{display:flex;align-items:center;flex:1;margin:0 20px 0 30px;min-width:0}
.progress-time{font-size:13px;color:var(--color-text);min-width:48px;flex-shrink:0}
.progress-container{position:relative;flex:1;height:8px;background:#e0e0e0;border-radius:4px;overflow:hidden;cursor:pointer;box-shadow:inset 0 1px 3px rgba(0,0,0,.1);user-select:none}
.progress-bar{height:100%;background:#ff5252;border-radius:4px;transition:width .12s ease;min-width:6px;box-shadow:0 0 8px rgba(255,82,82,.6)}
.progress-thumb{position:absolute;top:50%;left:0;width:16px;height:16px;background:#ff5252;border:3px solid #fff;border-radius:50%;transform:translate(-50%,-50%);box-shadow:0 2px 6px rgba(0,0,0,.3);opacity:0;transition:opacity .2s;pointer-events:none}
.progress-container:hover .progress-thumb,.progress-container:active .progress-thumb{opacity:1}
...
@media (max-width:768px){
  ...
  .progress-section{margin:0 10px 0 15px}
